        ...
        for index, download in enumerate(list(download_dict.values())[COUNT:], start=1):
            ...
            if download.status() not in [MirrorStatus.STATUS_SEEDING, MirrorStatus.STATUS_SPLITTING]:
                ...
                if download.message.chat.type != 'private':
                    ...
                    if EMOJI_THEME is True:
                        msg += f'\n<b>👤 User: </b> <code>{download.message.from_user.first_name}</code> | ⚠️ <code>/warn {download.message.from_user.id}</code>'
                        msg += f"\n<b>🚫 Cancel: </b><code>/{BotCommands.CancelMirror} {download.gid()}</code>"
                    else:
                        msg += f'\n<b>User: </b> <code>{download.message.from_user.first_name}</code> | <code>/warn {download.message.from_user.id}</code>'
                        msg += f"\n<b>Cancel: </b><code>/{BotCommands.CancelMirror} {download.gid()}</code>"
